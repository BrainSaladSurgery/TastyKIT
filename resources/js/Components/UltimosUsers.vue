<template>
    <div id="tickets" class="px-4">
        <section class="flex flex-col p-5 bg-gray-100 " id="card" >
            <h1 class="font-semibold mb-3 text-center">
                COMANDA #00011
            </h1>
                <article class="w-full  cursor-pointer border rounded-md p-3 bg-white flex text-gray-700 mb-2 hover:border-green-500 focus:outline-none focus:border-green-500">
                    <ul class=" w-full flex flex-col overflow-auto h-full">
                        <li class="w-full flex items-center justify-center py-2">
                            ...
                            <span> <b>Número de Ticket:</b> #11111 </span>
                        </li>
                        <hr />
                        <div v-for="(item,index) in items" @key="index">
                            <li class="w-full flex items-center justify-center py-3 mt-1">
                                <span   span class="w-1/3"> <b>- {{ item.name }}</b></span><span class="w-1/6 text-center">{{ item.amount }}Ud  </span><span class="w-1/6 text-center">{{ item.price }}€</span>
                            </li>
                        </div>
                        <hr />
                        <li class="w-full flex items-center justify-center pb-3 pt-5">
                            <span class="w-3/12 text-center"> <b>TOTAL</b></span><span class="w-1/6 text-center">80€</span>
                        </li>
                    </ul>
                </article>
        </section>

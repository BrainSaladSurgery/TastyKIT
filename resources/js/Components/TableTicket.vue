<template>
  <table aria-describedby="info-popup" aria-label="open tickets" class="border-t w-full min-h-0 h-full flex flex-col">
    <thead class="flex w-full flex-col px-4">
        <tr class="border-b grid grid-cols-8 gap-2 w-full justify-start py-3">
            ...
            <th id="Camarero" class="font-semibold text-center col-span-1 w-full items-center hidden md:block justify-center truncate">
                Camarero
            </th>
        ...
        <tbody  class="flex w-full flex-col min-h-0 overflow-hidden px-4 h-full">
            <tr v-for="(ticket,index) in tickets" @key="index" role="row" class="border-b grid grid-cols-8  gap-2  w-full h-15 justify-start items-center hover:bg-blue-100 cursor-pointer">
                ...
                <td headers="Camarero" class="text-center col-span-1 flex items-center justify-center w-full hidden md:block truncate">
                    {{ ticket.user }}
                </td>
                <td  headers="total" class="text-center col-span-1 flex items-center justify-center  w-full truncate">
                    {{ ticket.total }}€
                </td>
                <td  headers="status" class="text-left col-span-2 lg:col-span-1 flex items-center py-1  w-full truncate">
                
                    <button v-if="ticket.status == 'open'"  @click="changeModal(ticket.ticket)" class="w-36 border-2 border-transparent bg-orange  h-10 font-bold uppercase text-white rounded transition-all hover:border-red-blue hover:bg-yellow-700 hover:text-red-blue">Cerrar</button>
                    <button v-if="ticket.status == 'close'"  class="w-36  cursor-none border-2 border-transparent bg-gray-500 h-10 font-bold uppercase text-white rounded ">Cerrado</button>
                </td>
                    ...
                    <div class="bg-white rounded-lg w-96">
                        <div class="w-96 border-t-8 border-yellow-600 rounded-lg flex">
                            <div class="w-full pt-9 pr-4 text-center">
                                ...
                                <p class="py-4 text-sm text-gray-400">El Ticket <b>#00{{ numTick }}</b> se va a cobrar ¿Esta seguro?</p>
                            </div>
                        </div>
                        <div class="p-4 flex space-x-4">
                            ...
                            <button @click="closeTicket(numTick); changeModal()" class="w-1/2 px-4 py-3 text-center text-pink-100 bg-yellow-700 rounded-lg hover:bg-yellow-800 ">Cobrar</button>
                        </div>
                    </div>
                </modal>
                <!-- END MODAL CLOSE -->
